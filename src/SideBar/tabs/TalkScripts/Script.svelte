<script lang="ts">
	...
	function confirmDelete() {
		if (
			confirm(
				`Are you sure you want to delete the Script for ${script.objId}? This cannot be undone.`
			)
		) {
			...
			for (let lineList of task.lineLists) {
				for (let command of lineList) {
					if (command.EXECArgs?.TalkId) {
						let obj =
							'0x' +
							parseInt(command.EXECArgs?.TalkId || '')
								.toString(16)
								.padStart(4, '0');
						if (obj === script.objId) {
	...
	function getEmReference(emName: string, type: number): [string, Em] | null {
		...
		for (let emSet of $session.enemySet.sets) {
			for (let em of emSet.ems) {
				if (questLookup(em.Id, true)?.toUpperCase() === emName.toUpperCase() && em.SetType === type)
...
>
	<header>
		...
		<div class="text">
			...
			{:else}
				{#if emReference}
					<span translate="yes">{emReference[0].split(' > ')[0]}</span>
					<h3>{emReference[0].split(' > ')[1]}</h3>
				{:else}
					<h3>
						{script.objId}
						<span
							style="color: white; transform: rotate(5deg)"
							title="Can't find this object in this quest. Was it deleted?">
                            <IconHelp />
                        </span>
					</h3>
				{/if}
	...
	{#if expanded}
		...
		<div
			class="content"
			transition:slide={{ axis: 'y', duration: 100, easing: cubicInOut }}
			on:click={(e) => e.stopPropagation()}
		>
			<SelectNumberInput label="Triggered by" bind:value={script.triggerType}>
			...
			{#if script.triggerType !== 1}
				...
				{#if emReference}
					...
					<button
						...
						style="margin: 10px"
						on:click={() => {
							$currentEm = (emReference || [null, null])[1];
			...
			{:else}
				...
				/>
			{/if}
			<TextInput label="Quest ID" description="Defaults to qffff." bind:value={script.questId} />
