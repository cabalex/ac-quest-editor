<script lang="ts">
	...
	function addTalkScript(objId: string, triggerType: number) {
		...
		session.talkScript.scripts = [
			...session.talkScript?.scripts,
			new ScriptObject(objId, 0, 'qffff', triggerType)
		];
	}
...
<aside class="tabSidebar" transition:fly|global={{ x: -200, duration: 200, easing: cubicInOut }}>
	<header>
		...
		<div class="text">
			<h1>TalkScripts</h1>
			<span>Dialogue players can see and hear.</span>
		</div>
			...
			<Script
				...
				on:delete={() => {
					if (session.talkScript)
						session.talkScript.scripts = session.talkScript.scripts.filter((x) => x != script);
				}}
			/>
		{/each}
	...
	{:else}
		<div class="talkScriptSplash">
			<div class="text">
				<IconScript size={36} />
				<p>
					TalkScripts are how players interact with NPCs, allowing for dialogue choices, camera
					movements, emotes, and more.
				</p>
			</div>
...
{#if helpModalOpen}
	<Modal style="max-width: 750px" on:close={() => (helpModalOpen = false)}>
		...
		<p>
			Talk Scripts are how players interact with NPCs in Astral Chain. They can do as much as
			provide rich dialogue choices for the player to choose, to as little as displaying a speech
			bubble over someone's head.
		</p>
		<p>There are three ways a Talk Script can be triggered:</p>
		<ul>
			<li>
				<b>Through NPC Interaction</b>: Pressing A on the target NPC will trigger this Talk Script.
			</li>
			<li>
				<b>Through a Task</b>: A Task can trigger a Talk Script to run via its hexadecimal ID.
			</li>
			<li>
				<b>Through a Speech Bubble</b>: A speech bubble will appear over the target NPC's head,
				displaying conversation that doesn't need to interrupt gameplay.
			</li>
		</ul>
		<p>
			<b
				>NPC interaction and Speech Bubble TalkScripts can only be set on PL objects (e.g. PL2009,
				an NPC), not EMs.</b
			> You can't talk to chimeras :(
		</p>
	</Modal>
{/if}
