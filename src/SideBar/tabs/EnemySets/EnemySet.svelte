<script lang="ts">
	...
	import { writableMap } from '../../../Map/Map.svelte';
	...
	<div class="emSets" bind:this={elem}>
		...
		<div class="ems">
			{#each set.ems as em}
				<button class="em" class:active={$currentEm == em} on:click={focusEm.bind(null, em)}>
					...
					{#if em.SetType}
						<div
							class="setType"
							style={`background-color: hsl(${(em.SetType * 10) % 360}deg, 50%, 50%`}
						>
							<IconHash />
							{em.SetType}
						</div>
			...
			<button
				class="addBtn addEm"
				on:click={() => {
					set.ems = [...set.ems, new Em(set.ems.length, [0x20000, 0x20000])];
					focusEm(set.ems[set.ems.length - 1]);
				}}
			>
				<IconPlus />
				Add Enemy
		...
		{#if expanded}
			<div class="content" transition:slide={{ axis: 'y', duration: 100, easing: cubicInOut }}>
				...
				/>
				<TextInput label="Name" bind:value={set.name} />
				<BoolInput label="Can set" description="Unknown purpose." bind:value={set.CanSet} />
				<h2 class="sectionHeader" style="padding-left: 10px">
					Difficulties (unknown purpose, usually blank)
				</h2>
				<TextInput label="Easy" bind:value={set.easy} />
				<TextInput label="Normal" bind:value={set.normal} />
				<TextInput label="Hard" bind:value={set.hard} />
				<TextInput label="Very Hard" bind:value={set.very_hard} />
...
<style>
	...
	.referencedTasks::-webkit-scrollbar-track {
		background-color: #444;
	}
	.referencedTask {
		gap: 2px;
