...
export default class QuestData {
	...
	constructor(
		...
		if (!questFlags || questFlags.length !== 64) {
			// fallback
			this.questFlags = Array(64)
				.fill('')
				.map((_, i) => `Flag${i.toString().padStart(2, '0')}`);
		} else {
			this.questFlags = questFlags;
		}
		if (!saveFlags || saveFlags.length !== 64) {
			// fallback
			this.saveFlags = Array(64)
				.fill('')
				.map((_, i) => `SaveFlag${i.toString().padStart(2, '0')}`);
		} else {
...
export class TaskList {
	...
	static fromNode(node: Node) {
		return new TaskList(
			...
			node.children
				.find((x) => x.name == 'LineListTree')
				?.children.map((child: Node) =>
					child.children.map((commandList) => Command.fromNode(commandList))
				) || []
		);
	}
	...
	repack() {
		const node: Node = {
			...
			children: [
				...
				...Object.keys(this.IFArgs).map((key) => {
					return { name: key, attributes: {}, value: this.IFArgs[key], children: [] };
				}),
				{
					name: 'typeEXEC',
					attributes: {},
				...
				...Object.keys(this.EXECArgs).map((key) => {
					return { name: key, attributes: {}, value: this.EXECArgs[key], children: [] };
				})
			]
		};
...
export class Area {
	...
	static fromNode(node: Node) {
		...
		let options = {
			center: new Vector(),
			points: [new Vector(), new Vector(), new Vector(), new Vector()] as [
				Vector,
				Vector,
				Vector,
				Vector
