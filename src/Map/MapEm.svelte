<script lang="ts">
	...
	$: collision = getCollision(questLookup(em.Id, true));
	...
	$: {
		if (
			...
			(cachedEm === em && cachedPos !== cachedEm.Trans.repack())
		) {
			cachedPos = em.Trans.repack();
...
>
	...
	{#if collision}
		<div
			class="collision"
			style={`width: ${(collision[1][0] - collision[0][0]) * 2}px; height: ${(collision[1][2] - collision[0][2]) * 2}px`}
		/>
	{/if}
</div>
