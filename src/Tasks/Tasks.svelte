<script lang="ts">
	...
	function saveTaskList() {
		...
		if (
			dom.querySelector(':scope > block[type="unknown-if"]') ||
			dom.querySelector(':scope > block[type="unknown-exec"]')
		) {
		...
		function parseCommands(elem: Element): Command[] {
            ...
            */
			...
			for (let field of elem.querySelectorAll(':scope > field')) {
				...
				if (typeIF !== 0) {
					let inputField = ifBlocks
						.find((b) => b.type === `if-${typeIF}`)
						?.args0.find((f: any) => f.name === name);
					if (inputField?.type === 'field_input' || isHex(name)) {
						value = parseInt(value || '0', 16).toString();
					}
					IFArgs[name] = value?.toString();
				} else {
					let inputField = execBlocks
						.find((b) => b.type === `exec-${typeEXEC}`)
						?.args0.find((f: any) => f.name === name);
					if (inputField?.type === 'field_input' || isHex(name)) {
						value = parseInt(value || '0', 16).toString();
					}
			...
			if (elem.querySelector(':scope > statement[name="execarea"]') && typeIF !== 0) {
				...
				if (!nextBlock) {
					...
					nextBlock?.querySelector(':scope > next > *') ||
					nextBlock?.getAttribute('type')?.toLowerCase().startsWith('if')
				) {
			...
			if (nextBlock) {
				if (typeIF !== 0 && typeEXEC === 1) {
					...
					(typeEXEC = 1), commands.push(new Command(0, {}, 0, {}));
				}
				commands.push(...parseCommands(nextBlock));
			}
			...
			if (typeIF !== 0 && typeEXEC === 0) {
				...
				(typeEXEC = 1), commands.unshift(new Command(0, {}, 0, {}));
			}
		...
		for (let task of dom.querySelectorAll(':scope > block[type="task-start"]')) {
			let taskNo = parseInt(
				task.querySelector(':scope > field[name="taskno"]')?.textContent || '100'
			);
	...
	const isHex = (key: string): boolean => {
		return key.includes('Hash') || key.includes('QuestId') || key.includes('PhaseNo');
	};
                ...
                */
				if (Object.keys(blockIds).includes('if-' + command.typeIF)) {
					...
					for (let [key, value] of Object.entries(command.IFArgs)) {
						...
						let inputField = ifBlocks
							.find((b) => b.type === `if-${command.typeIF}`)
							?.args0.find((f: any) => f.name === key);
						if (inputField?.type === 'field_input' || isHex(key)) {
				...
				if (Object.keys(blockIds).includes('exec-' + command.typeEXEC)) {
					...
					for (let [key, value] of Object.entries(command.EXECArgs)) {
						let inputField = execBlocks
							.find((b) => b?.type === `exec-${command.typeEXEC}`)
							?.args0.find((f: any) => f.name === key);
						if (inputField?.type === 'field_input' || isHex(key)) {
							value = parseInt(value).toString(16);
						}
	...
	let cachedDom: Element | null = null;
	function onChange() {
		let currentDom = Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace());
		if (cachedDom && currentDom && !cachedDom.isEqualNode(currentDom)) {
			// no changes detected
			console.log('Changes detected, saving...');
			cachedDom = currentDom;
			let saved = saveTaskList();
			if (
				saved &&
				$session instanceof Quest &&
				$currentTask !== null &&
				$session.questData.tasks[$currentTask]
			)
	...
	onMount(() => {
		...
		render();
		cachedDom = Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace());
	});
...
<div class="taskEditor" transition:fly|global={{ duration: 200, x: 200, easing: cubicInOut }}>
	<BlocklyComponent {config} locale={en} bind:workspace on:change={onChange} />
</div>
